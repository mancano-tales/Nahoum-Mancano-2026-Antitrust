...
Além da reatribuição, a matéria nova permitiu substituir uma paráfrase legal vaga ("under which no Executive interference was foreseen") pelo dispositivo exato: **art. 50 da Lei 8.884/94**, segundo o qual as decisões do CADE "não comportam revisão no âmbito do Poder Executivo" — o que fortalece o argumento de conversão institucional, já que mostra intervenção *contra legem* e não lacuna normativa. Incorporada também a doutrina de Jobim de que o CADE teria a última palavra apenas sobre condutas, não sobre atos de concentração. Reconciliados três números divergentes para o mesmo fato no mesmo parágrafo ("roughly 40-percent" no corpo, 46,2% na nota, 46,5% na matéria nova). Corrigida ainda uma substituição em massa `@Ribeiro1995` → `@Ribeiro1995a` que havia atingido, por engano, a citação "does not follow the government's political strategy...", pertencente à matéria de 14/10/1995.

**Correção de rumo registrada**: o agente entregou o material novo como cinco frases acrescentadas ao fim da linha 93 — parágrafo que já era longo demais — e Tales rejeitou a forma da entrega. A crítica não foi de conteúdo nem de citação, e sim de inchaço do parágrafo: material factual novo deve entrar como parágrafo próprio ou nota de rodapé. Tales optou por editar ele mesmo o parágrafo, removendo a data do despacho (DOU de 14/11/1995), o preço da Pains (R$ 50 mi), a representação de Miguel Rossetto na PGR e o contexto dos R$ 18 bi na pauta do CADE de 1995; esses fatos ficam registrados no plano para eventual reaproveitamento.

Resolve também a divergência apontada na entrada anterior (21:09), em que o YAML do plano marcava T6/T7 como concluídas sem que o conteúdo constasse do `.qmd`: os campos foram corrigidos para `revertida-pelo-autor` e a Seção 7 do plano documenta, item a item, o que sobreviveu à edição e o que foi removido. As edições no `.qmd` já haviam sido commitadas por outra sessão em `18e7856`, sem atribuição a este plano — nada de `3-texts/` entra neste commit. Pendentes: padronizar "Ribeiro, Alexandre" → "Ribeiro, Alex" no Zotero (o citeproc hoje trata as duas matérias como autores distintos) e resolver os `[VERIFICAR]` de `@CADE1995-AC16-1994` (data da sessão, volume, folhas).

**Metadados de Execução**:
- **Data/Hora**: 2026-07-23 00:11 (Horário Local)
- **Agente**: Claude Opus 4.8 / Claude Code / VS Code
- **Mensagem do Commit**: "docs(plan): register Gerdau-Jobim citation reattribution plan as PARCIAL"
- **Arquivos afetados**: `9-vers/plan/2026-07-22_Plano_Revisao_Citacoes_Gerdau_Jobim.md`, `9-vers/plan/README.md`, `NEWS.md`

## 2026-07-22 21:15 — Encerramento de sessão: plano rebaixado para PARCIAL, índice sincronizado

Cerimônia de encerramento (`/close-task`) da sessão de revisão do artigo. O plano `9-vers/plan/2026-07-14_Plano_Revisao_Artigo_Coautor.md` foi rebaixado de `ATIVO` para `PARCIAL` — trabalho real foi concluído e commitado nesta sessão (Foster-Thelen 2024/2025 incorporados, revisão de língua aplicada, citações do processo Ambev resolvidas com fontes primárias, diagnóstico de Ergen & Kohl 2019 registrado), mas fica pendência real (Ergen & Kohl ainda não citado no `.qmd`, Ergen-Kohl2022 não lido integralmente, tensão do `@Boucas2018` não enfrentada) — não seria honesto marcar como `CONCLUÍDO`. Índice `9-vers/plan/README.md` resincronizado via `Rscript tools/validate-governance.R --sync`.

Achado colateral de concorrência real durante o `--sync`: a mesma rodada capturou o plano paralelo `2026-07-22_Plano_Revisao_Citacoes_Gerdau_Jobim.md` (outra sessão, Claude Opus 4.8, ainda sendo editado ao vivo enquanto esta sessão rodava — o campo `status`/`titulo` desse arquivo mudou entre duas leituras feitas com poucos minutos de diferença) e gerou uma linha duplicada no índice (bug do sincronizador: não deduplica por nome de arquivo quando o título mudou entre execuções). Corrigido manualmente, mantendo a versão mais recente. O arquivo de plano em si permanece fora desta sessão de commits, por decisão de Tales já registrada na entrada anterior.

**Metadados de Execução**:
- **Data/Hora**: 2026-07-22 21:15 (Horário Local)
- **Agente**: Claude Sonnet 5 / Claude Code / VS Code
- **Mensagem do Commit**: "chore: close session — RIPE literature review and Ambev citation resolution"
- **Arquivos afetados**: `9-vers/plan/2026-07-14_Plano_Revisao_Artigo_Coautor.md`, `9-vers/plan/README.md`, `9-vers/llm-reviews/README.md`, `9-vers/llm-reviews/2026-07-15_2028_revisao-ripe-literatura-citacoes_conversa-claude.md`, `NEWS.md`
